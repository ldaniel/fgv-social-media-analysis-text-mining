...
nodes <- data.frame(id = seq(1:35),
                    name = c(rede_two_mode$nome, names(rede_two_mode)))

# identifica os grupos
nodes$group <- sample(grupos, 35, replace = TRUE)
...
edges$from <- as.integer(plyr::mapvalues(edges$from, nodes$name, nodes$id, warn_missing = FALSE)) - 1
edges$to <- as.integer(plyr::mapvalues(edges$to, nodes$name, nodes$id, warn_missing = FALSE)) - 1
